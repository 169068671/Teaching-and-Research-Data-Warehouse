...
DEFAULT_REMOTE = "git@github.com:169068671/Teaching-and-Research-Data-Warehouse.git"
DEFAULT_BRANCH = "main"
...
def normalize_remote(value: str) -> str:
    ...
    match = re.fullmatch(r"ssh://(?:[^@]+@)?([^/]+)/(.+)", remote, flags=re.I)
    if match:
        return f"{match.group(1).lower()}/{match.group(2).lower()}"
...
def remote_transport(value: str) -> str:
    remote = value.strip()
    if re.match(r"^(?:git@[^:]+:|ssh://)", remote, flags=re.I):
        return "ssh"
    if re.match(r"^https?://", remote, flags=re.I):
        return "https"
    return "local"
...
def validate_vault(vault: Path) -> dict[str, object] | None:
    validators = [vault / "99_System" / "validate_vault.py", vault / "99_系统" / "validate_vault.py"]
    validator = next((path for path in validators if path.is_file()), None)
    if validator is None:
        return {"status": "SKIPPED", "reason": "知识库未配置核验脚本"}
...
def ensure_remote(vault: Path, remote_name: str, remote_url: str) -> tuple[bool, bool]:
    ...
    migrated = remote_transport(existing) == "https" and remote_transport(remote_url) == "ssh"
    if migrated:
        git(vault, ["remote", "set-url", remote_name, remote_url])
...
def friendly_error(message: str) -> str:
    ...
    if any(marker in lower for marker in auth_markers):
        return (
            "GitHub SSH 认证失败。请把本机 SSH 公钥添加到 GitHub，然后在终端执行：\n"
            "ssh -T git@github.com\n"
            "验证成功后再点击同步。插件不会读取或保存私钥。"
        )
    if "host key verification failed" in lower:
        return "GitHub SSH 主机校验失败。请先在终端执行 ssh -T git@github.com，确认主机指纹后重试。"
...
def status(vault: Path, remote_name: str) -> dict[str, object]:
    ...
    return {
        ...
        "branch": branch,
        "remote": remote,
        "remote_transport": remote_transport(remote) if remote else None,
        "changes": len(changes),
...
def test_connection(vault: Path, remote_url: str, branch: str) -> dict[str, object]:
    if remote_transport(remote_url) != "ssh":
        raise SyncError(f"当前配置不是 SSH 地址：{remote_url}")
    if not is_git_repo(vault) or not has_head(vault):
        raise SyncError("当前知识库还没有可测试的 Git 提交。")
    ...
    if read_test.returncode != 0:
        raise SyncError((read_test.stderr or read_test.stdout).strip() or "无法读取 GitHub SSH 仓库。")

    push_test = git(
        vault,
        ["push", "--dry-run", remote_url, f"HEAD:refs/heads/{branch}"],
        check=False,
    )
    if push_test.returncode != 0:
        detail = (push_test.stderr or push_test.stdout).strip()
        raise SyncError(f"SSH 读取成功，但推送预演失败；没有上传任何内容。\n{detail}")
    ...
    return {
        ...
        "branch": branch,
        "remote_branch_exists": bool(read_test.stdout.strip()),
        "push_dry_run": True,
        "message": "GitHub SSH 读写权限和推送预演均通过；没有上传任何内容。",
    }
...
def sync(
    ...
    ensure_branch(vault, branch)
    remote_added, remote_migrated = ensure_remote(vault, remote_name, remote_url)
    ...
    return {
        ...
        "remote_migrated_to_ssh": remote_migrated,
        ...
        "commit": commit,
        "remote": remote_url,
        "remote_transport": remote_transport(remote_url),
        "upstream_preserved": upstream or None,
...
def main() -> int:
    ...
    try:
        if args.status:
            ...
            result = test_connection(vault, args.remote_url, args.branch)
        else:
            result = sync(
                vault,
                args.remote_url,
